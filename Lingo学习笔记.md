...
* 参数的数量级要适当
* 尽量使用线性模型，少使用绝对值
* 少使用符号函数，多变量求最大最小值，取整函数等非线性函数
* @sos1('setname',x),限制x中至多一个大于0，p25
* @card('setname',x):限制x中非零元素的最多个数
* @semic(L,x,U):限制x=0或x在L,U，lingo12才有的
* @text()=@table(x);以表格的形式把x的计算结果输出到屏幕
...
## lingo定义子模型
    ...
    calc:
        @solve(submodel_name1);
    endcalc

需要把求解器设置为全局求解器。

### 例：求背包问题的多个解
将原来的求解代码封装到一个子模型里面，不断进行调用。
但是在后面的调用的时候，采用“割”的方法，即增加一些约束条件，排除掉已经求得的解。

书上的一个方法适用于0-1变量问题。P46 
